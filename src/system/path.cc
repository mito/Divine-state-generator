...
static string get_trans_string(explicit_system_t* System, state_t s1, state_t s2, bool with_prop) {
...
void path_t::write_trans(std::ostream & out) {
  for(unsigned i=0; i<s_list.size(); i++) {
    ...
    if (i+1 < s_list.size()) {
      out << get_trans_string(System, s_list[i], s_list[i+1], false) << endl;
    }
  }
  if (cycle_start_index != -1) {
    ...
    if (System->get_succs(s_list[cycle_start_index], succs) != SUCC_ERROR)
	  ...
	  out << get_trans_string(System, s_list[s_list.size()-1], s_list[cycle_start_index], false) << endl;
      }
