...
public class VTextIcon implements Icon, PropertyChangeListener {
    ...
    void calcDimensions() {
       // Graphics g = fComponent.getGraphics();
       // if ( g == null ) return;
       // FontRenderContext fc = ((Graphics2D) g).getFontRenderContext();
		FontMetrics fm = fComponent.getFontMetrics(fComponent.getFont());
        fCharHeight = fm.getHeight();
        // Font f = fComponent.getFont();
        // TextLayout tl = new TextLayout( fLabel, f, fc );
        // fCharHeight = (int) tl.getVisualHighlightShape( b, e ).getBounds().getHeight();
        // fCharHeight = 1;
        // fCharHeight = (int) f.getLineMetrics( fLabel, fc ).getHeight();
        // fCharHeight = (int) fm.getStringBounds( fLabel, g ).getHeight();
   ...
   /**
	...
	/** 
	 ...
	 is rotated by 90 deg counterclockwise so that the characters 
	 are ordered from top to bottom. Latin text and numbers 
	 may be rotated 90 deg clockwise so that the characters 
