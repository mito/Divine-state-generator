...
int main(int argc, char** argv) {  
  ...
  bool print_acc= false, print_trans=false, print_dot=false, print_help=false,
       ...
       print_deadlock=false, print_leading_to_error=false, compiled_generator = true;
      ...
      if (filename_length>=2 && strcmp(filename+filename_length-2,".b")==0)
       {
        ...
        p_System = new bymoc_explicit_system_t(gerr);
       }
      else
       {
         cout << "Reading ";
         if(compiled_generator)
         {
           p_System = new dveC_explicit_system_t(gerr);
           if (strlen(argv[optind])>=4 && strcmp(filename+strlen(argv[optind])-4,".dve")==0)
           {
	     cout << "and compiling ";
           }
         }
